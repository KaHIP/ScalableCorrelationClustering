...
#include "clustering/uncoarsening/refinement/quotient_graph_refinement/partial_boundary.h"
...
EdgeWeight two_way_fm::perform_refinement(PartitionConfig & cfg, 
        ...
        partition_accept_rule* accept_partition = new normal_partition_accept_rule(config, cut,lhs_part_weight, rhs_part_weight);
        queue_selection_strategy* q_select      = new queue_selection_topgain(config);
        ...
        for(number_of_swaps = 0; number_of_swaps < max_number_of_swaps; number_of_swaps++) {
                ...
                if(!from_queue->empty()) {
                        ...
                        if(accept_partition->accept_partition(config, cut, lhs_part_weight, rhs_part_weight, pair->lhs, pair->rhs)) {
                                ASSERT_TRUE( cut <= best_cut);
                                if( cut < best_cut ) {
        ...
        ASSERT_EQ(cut, qm.edge_cut(G, pair->lhs, pair->rhs));
        ASSERT_TRUE(assert_directed_boundary_condition(G, boundary, pair->lhs, pair->rhs)); 
        ASSERT_TRUE((int)inital_cut-(int)best_cut >= 0);
...
void two_way_fm::move_node(const PartitionConfig & config, 
        ...
        if(is_boundary_node(G, node, to, from)) {
        ...
        forall_out_edges(G, e, node) {
                ...
                bool boundary_node = is_boundary_node(G, target, targets_partition, other_partition);
...
void two_way_fm::init_queue_with_boundary(const PartitionConfig & config,
        ...
        for( unsigned int i = 0, end = bnd_nodes.size(); i < end; i++) {
                ...
                ASSERT_TRUE(is_boundary_node(G, cur_bnd_node, partition_of_boundary, other));
...
bool two_way_fm::assert_only_boundary_nodes(graph_access & G, PartialBoundary & lhs_boundary, 
        ...
        forall_boundary_nodes(lhs_boundary, cur_bnd_node) {
                ...
                ASSERT_TRUE(is_boundary_node(G, cur_bnd_node, lhs, rhs));
        } endfor
        return true;
}
...
bool two_way_fm::assert_every_boundary_nodes(graph_access & G, PartialBoundary & lhs_boundary, 
        ...
        forall_nodes(G, n) {
                ...
                if(G.getPartitionIndex(n) == lhs) {
                        ...
                        if(is_boundary_node(G, n, lhs, rhs)) {
